...
class Settings:
    ...
    voices_dir: Path = field(default_factory=lambda: _env_path(
        "NEUROVOICE_VOICES_DIR", _repo_root() / "configs" / "voices"
    ))
    reference_audio_dir: Path = field(default_factory=lambda: _env_path(
        "NEUROVOICE_REFERENCE_DIR", _repo_root() / "data" / "reference-audio"
    ))
    ...
    # every inference records the resolved revision so a quality drift
    # can be traced to "did the model file change underneath us" rather
    # than guessing.
    model_hf_revision: str = field(default_factory=lambda: os.environ.get(
        "NEUROVOICE_MODEL_HF_REVISION", "main"
    ))
    lora_path: Path | None = field(default_factory=lambda: _env_path(
        "NEUROVOICE_LORA_PATH", Path(os.environ["NEUROVOICE_LORA_PATH"])
    ) if os.environ.get("NEUROVOICE_LORA_PATH") else None)
    lora_config_path: Path | None = field(default_factory=lambda: _env_path(
        "NEUROVOICE_LORA_CONFIG_PATH", Path(os.environ["NEUROVOICE_LORA_CONFIG_PATH"])
    ) if os.environ.get("NEUROVOICE_LORA_CONFIG_PATH") else None)
    device: str = field(default_factory=lambda: os.environ.get("NEUROVOICE_DEVICE", "auto"))
    optimize: bool = field(default_factory=lambda: _env_bool("NEUROVOICE_OPTIMIZE", False))
    cfg_value: float = field(default_factory=lambda: float(
        os.environ.get("NEUROVOICE_CFG_VALUE", "2.0")
    ))
    inference_timesteps: int = field(default_factory=lambda: int(
        os.environ.get("NEUROVOICE_INFERENCE_TIMESTEPS", "10")
    ))
    max_chars_per_request: int = field(default_factory=lambda: int(
        os.environ.get("NEUROVOICE_MAX_CHARS", "4000")
    ))
    # async long-form ceiling. The Pydantic schema
    ...
    async_max_chars: int = field(default_factory=lambda: int(
        os.environ.get("NEUROVOICE_ASYNC_MAX_CHARS", "100000")
    ))
    reference_trim_seconds: float = field(default_factory=lambda: float(
        os.environ.get("NEUROVOICE_REF_TRIM_SECONDS", "15.0")
    ))
    ...
    reference_sample_rate: int = field(default_factory=lambda: int(
        os.environ.get("NEUROVOICE_REF_SR", "16000")
    ))
    target_sample_rate: int = field(default_factory=lambda: int(
        os.environ.get("NEUROVOICE_TARGET_SR", "48000")
    ))
    api_keys: list[str] = field(default_factory=lambda: _env_list("NEUROVOICE_API_KEYS", []))
    require_auth: bool = field(default_factory=lambda: _env_bool("NEUROVOICE_REQUIRE_AUTH", True))
    # CORS allow-list. Default targets local dev (gateway + an SPA on
    # 5173). Production deployments MUST set NEUROVOICE_CORS_ORIGINS to the
    ...
    # the floor via NEUROVOICE_ENROLL_MIN_SECONDS once they wire real FSEK
    ...
    enroll_min_seconds: float = field(default_factory=lambda: float(
        os.environ.get("NEUROVOICE_ENROLL_MIN_SECONDS", "1.0")
    ))
    ...
    tenant_rate_limit_per_minute: int = field(default_factory=lambda: int(
        os.environ.get("NEUROVOICE_TENANT_RATE_LIMIT_PER_MINUTE", "600")
    ))
